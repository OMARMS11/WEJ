...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/weja-target";
    ...
    results = [
      {
        ...
        description: "Sample search result item",
      },
      {
        id: 2,
        title: `Another match for "${query}"`,
        description: "More sample content",
      },
    ];
  }

  if (dbConnected) {
    recentSearches = await SearchLog.find()
      .sort({ createdAt: -1 })
      .limit(5)
      .catch(() => []);
  }
...
app.post("/login", (req, res) => {
  ...
  res.render("login", {
    ...
    message: {
      type: "success",
      text: `Login attempt with username: ${username}`,
    },
    dbStatus: getDbStatus(),
  });
});
